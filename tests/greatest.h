...
/* Set to 0 to disable ANSI colors in test output. */
#ifndef GREATEST_USE_COLOR
#define GREATEST_USE_COLOR 1
#endif

#if GREATEST_USE_COLOR
#define GREATEST_COLOR_RED "\x1b[31m"
#define GREATEST_COLOR_GREEN "\x1b[32m"
#define GREATEST_COLOR_YELLOW "\x1b[33m"
#define GREATEST_COLOR_RESET "\x1b[0m"
#else
#define GREATEST_COLOR_RED ""
#define GREATEST_COLOR_GREEN ""
#define GREATEST_COLOR_YELLOW ""
#define GREATEST_COLOR_RESET ""
#endif
...
static void greatest_do_pass(void) {                                    \
    ...
    if (GREATEST_IS_VERBOSE()) {                                        \
        GREATEST_FPRINTF(GREATEST_STDOUT, "%sPASS%s %s: %s",            \
            GREATEST_COLOR_GREEN, GREATEST_COLOR_RESET,                 \
            g->name_buf, g->msg ? g->msg : "");                         \
    } else {                                                            \
        GREATEST_FPRINTF(GREATEST_STDOUT, "%s.%s",                      \
            GREATEST_COLOR_GREEN, GREATEST_COLOR_RESET);                \
    }                                                                   \
    g->suite.passed++;                                                  \
}                                                                       \
...
static void greatest_do_fail(void) {                                    \
    ...
    if (GREATEST_IS_VERBOSE()) {                                        \
        GREATEST_FPRINTF(GREATEST_STDOUT,                               \
            "%sFAIL%s %s: %s (%s:%u)",                                 \
            GREATEST_COLOR_RED, GREATEST_COLOR_RESET,                   \
            g->name_buf, g->msg ? g->msg : "",                          \
            g->fail_file, g->fail_line);                                \
    } else {                                                            \
        GREATEST_FPRINTF(GREATEST_STDOUT, "%sF%s",                      \
            GREATEST_COLOR_RED, GREATEST_COLOR_RESET);                  \
...
static void greatest_do_skip(void) {                                    \
    struct greatest_run_info *g = &greatest_info;                       \
    if (GREATEST_IS_VERBOSE()) {                                        \
        GREATEST_FPRINTF(GREATEST_STDOUT, "%sSKIP%s %s: %s",            \
            GREATEST_COLOR_YELLOW, GREATEST_COLOR_RESET,                \
            g->name_buf, g->msg ? g->msg : "");                         \
    } else {                                                            \
        GREATEST_FPRINTF(GREATEST_STDOUT, "%ss%s",                      \
            GREATEST_COLOR_YELLOW, GREATEST_COLOR_RESET);               \
    }                                                                   \
    g->suite.skipped++;                                                 \
}                                                                       \
...
static void report_suite(void) {                                        \
    if (greatest_info.suite.tests_run > 0) {                            \
        GREATEST_FPRINTF(GREATEST_STDOUT,                               \
            "\n%u test%s - %s%u passed%s, %s%u failed%s, %s%u skipped%s",\
            greatest_info.suite.tests_run,                              \
            greatest_info.suite.tests_run == 1 ? "" : "s",              \
            GREATEST_COLOR_GREEN, greatest_info.suite.passed,           \
            GREATEST_COLOR_RESET,                                       \
            GREATEST_COLOR_RED, greatest_info.suite.failed,             \
            GREATEST_COLOR_RESET,                                       \
            GREATEST_COLOR_YELLOW, greatest_info.suite.skipped,         \
            GREATEST_COLOR_RESET);                                      \
...
void GREATEST_PRINT_REPORT(void) {                                      \
    if (!GREATEST_LIST_ONLY()) {                                        \
        ...
        GREATEST_FPRINTF(GREATEST_STDOUT,                               \
            "Pass: %s%u%s, fail: %s%u%s, skip: %s%u%s.\n",              \
            GREATEST_COLOR_GREEN, greatest_info.passed,                 \
            GREATEST_COLOR_RESET,                                       \
            GREATEST_COLOR_RED, greatest_info.failed,                   \
            GREATEST_COLOR_RESET,                                       \
            GREATEST_COLOR_YELLOW, greatest_info.skipped,               \
            GREATEST_COLOR_RESET);                                      \
    }                                                                   \
}                                                                       \
                                                                        \
